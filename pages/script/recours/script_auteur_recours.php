...
if (isset($_SESSION['utilisateur']) && $_SESSION['utilisateur']['nom_role'] == 'admin') {
    try {
        ...
        // Préparation de la première requête SQL
        $stmt1 = $bdd->prepare($sql1);
        $stmt1->bindParam(':id_dossier', $dossierId, PDO::PARAM_INT);
        $stmt1->execute();
        $contrevenants = $stmt1->fetchAll(PDO::FETCH_ASSOC);

        // Requête SQL supplémentaire à exécuter pour récupérer tous les enregistrements de la table parquet
        $sql2 = "SELECT * FROM parquet"; // Modifier la requête selon vos besoins

        // Préparation de la deuxième requête SQL
        $stmt2 = $bdd->prepare($sql2);
        $stmt2->execute();
        $autreResultat = $stmt2->fetchAll(PDO::FETCH_ASSOC);
        ...
        echo json_encode(array("success" => true, "contrevenants" => $contrevenants, "autreResultat" => $autreResultat));
